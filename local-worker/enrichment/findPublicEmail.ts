...
const buildSearchPrompt = (input: EnrichmentInput): string => {
  ...
  const lines = [
    ...
    "  e) \"<nome regista>\" instagram OR linktr.ee  (la bio del profilo Instagram/Linktree a volte contiene l'email di contatto pubblica)",
    "",
    "STEP 3 — APRI LE PAGINE E LEGGI",
    "Per ogni risultato promettente (sito ufficiale, IMDb, FilmFreeway, Vimeo del regista, sito della produzione, sito del festival, profilo Instagram del regista o suo Linktree — la bio può contenere un'email di contatto), USA IL WEB FETCH TOOL per caricare la pagina e LEGGERE il contenuto. NON inventare email basandoti solo sul nome del dominio.",
    "",
    "STEP 4 — VERIFICA",
